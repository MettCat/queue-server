...
import com.nettyserversql.bigqueue.page.IMappedPage;
import com.nettyserversql.bigqueue.page.IMappedPageFactory;
import com.nettyserversql.bigqueue.page.MappedPageFactoryImpl;
import com.nettyserversql.bigqueue.utils.Calculator;
import com.nettyserversql.bigqueue.utils.FileUtil;
 ...
 *  
 * Main features:主要特点:
 * 快:接近直接存储器存取的速度，在附加和顺序读取模式下，速度极快， 顺序附加和读取接近O(1)内存访问，随机读取接近O(1)内存访问 数据在缓存中，如果数据不在缓存中，则接近O(1)磁盘访问。
...
public class BigArrayImpl implements IBigArray {
	...
	// folder name for index page索引页的文件夹名称
	...
	// seconds, time to live for index page cached in memory  秒，用于存储在内存中的索引页面的时间
...
//	private final static int INDEX_ITEM_DATA_ITEM_OFFSET_OFFSET = 8;
	...
	// head index of the big array, this is the read write barrier. 大数组的头索引，这是读写屏障。
	//读者只能在此索引之前读取项，而写入可以写入该索引或之后
