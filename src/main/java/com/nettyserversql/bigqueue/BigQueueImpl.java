 ...
 * <p/>
 * Main features:
 * 快:接近直接内存访问的速度，队列和dequeue都接近O(1)内存访问。
...
public class BigQueueImpl implements IBigQueue {
    ...
    // size in bytes of queue front index page队列前索引页的字节数
    final static int QUEUE_FRONT_INDEX_PAGE_SIZE = 1 << QUEUE_FRONT_INDEX_ITEM_LENGTH_BITS;
    // only use the first page只使用第一个页面
    static final long QUEUE_FRONT_PAGE_INDEX = 0;

    // folder name for queue front index page队列前端索引页的文件夹名称
    ...
    // factory for queue front index page management(acquire, release, cache)用于队列前端索引页面管理的工厂
    IMappedPageFactory queueFrontIndexPageFactory;

    // locks for queue front write management队列前端写管理锁
     ...
     * @param queueName the name of the queue, will be appended as last part of the queue directory 队列的名称将被追加到队列目录的最后一部分
